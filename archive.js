// archive.js -- page scraping, response parsing and the original "review" play mode.
// This is j-play 1.4.0 (Wayne Davison) with small additions marked "j-play-live".
var contestants = { };
var categories = [ ];
var clues = [ ];
var nicknames = [ ]; // j-play-live: exposed for live.js (podium order, left to right)
...
function initialize()
{
    ...
    document.addEventListener('keydown', function(e) {
        if (typeof liveActive !== 'undefined' && liveActive) // j-play-live handles its own keys
            return true;
...
function createButtons(containing_el, dj_el, fj_el)
{
    ...
    let html = '<br><table align="center" valign="middle"><tr><td>'
             + '<button id="live_btn" class="pLaYliveBtn">&#9654; Play Live</button>'
...
function delayedInit()
{
    ...
    let onclicks = {
        live_btn: function() { if (typeof startLiveGame === 'function') startLiveGame(); },
...
function finishClueParsing(clue)
{
    if (!clue.response) {
        ...
        // j-play-live: keep the structured facts the live mode needs.
        clue.right = right.slice();
        clue.wrong = wrong.slice();
        clue.triple_stumper = triple_stumper;
        clue.correct_response = ans;
        clue.abbrev_response = abbrev_ans;
        clue.raw_response_html = html;
...
function checkClueResize()
{
    if (clueDiv.needResize) {
        ...
        if (tbl_el) {
            let _x = 0;
            let _y = 0;
            for (let el = tbl_el; el && !isNaN(el.offsetLeft) && !isNaN(el.offsetTop); el = el.offsetParent) {
